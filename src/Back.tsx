...
export async function search(keyword: string) {
  ...
  try {
    ...
    if (response.data.status === "ok") {
      const filteredStations = response.data.data.filter(
        (station: {
          station: { name: string };
          aqi: number;
          uid: number;
...
export async function showStation(uid: number) {
  let stationInfo = {
    pm25: { v: -999 },
    pm10: { v: -999 },
    o3: { v: -999 },
  ...
  const apiUrl =
    `https://api.waqi.info/feed/@` + uid.toString() + `/?token=` + token();
...
function upToDate(vtime: number) {
