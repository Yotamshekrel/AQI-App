...
function App() {
  //MainPost
  const userLocation = useLocation() as unknown as { city?: string };
  const [location, setLocation] = useState(
    userLocation?.city !== undefined ? userLocation?.city : "Loading..."
  );
  ...
  const listSwap = (listIndex: number) => {
    ...
    setLocation(
      stationsList[0].station.station.name.substring(
        0,
        stationsList[0]?.station.station.name.indexOf(",")
      )
    );
    setStationInfo({
  ...
  return (
    <>
      ...
      <MainPost
        index={aqi}
        city={location || "Unknown City"}
        data={stationInfo}
        onClickInfoFunc={() => onClickInfoFunc()}
        onClickListFunc={() => onClickListFunc()}
      ...
      <div className="linkedin-wrapper">
        <a
          ...
          href="https://www.linkedin.com/in/yotam-shekrel/"
          target="_blank"
        >
          <img className="linkedin-logo" src="/icons/linkedin-logo.png" />
